...
unsigned char LpcdRegisterInit(void)
{
	...
	ret = spi_SetReg(ComIEnReg,regdata);
	IF_ERR_THEN_RETURN;

	regdata = DIVIEN_DEF;			 
	ret = spi_SetReg(DivIEnReg,regdata);
	IF_ERR_THEN_RETURN;                //配置IRQ引脚作为标准CMOS输出，覵Tatus1Reg的IRQ位相反*/
