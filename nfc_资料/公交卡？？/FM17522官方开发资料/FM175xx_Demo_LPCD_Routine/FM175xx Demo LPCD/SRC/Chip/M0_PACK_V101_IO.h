...
//! 定义P2.7 为LED_RED	  
#define IO_MASK_LED_RED		(1 << 7)															
#define LED_RedCfg()		LPC_GPIO2->DIR |= IO_MASK_LED_RED									//!< 配置LED_RED 为GPIO 输出
#define LED_RedOff()		(LPC_GPIO2->MASKED_ACCESS[IO_MASK_LED_RED] = IO_MASK_LED_RED)		//!< 关闭LED_RED
#define LED_RedOn()			(LPC_GPIO2->MASKED_ACCESS[IO_MASK_LED_RED] = ~IO_MASK_LED_RED)		//!< 点亮LED_RED
...
#define IO_MASK_IRQ			(1 << 7)												//!< CD中断IRQ脚掩码
//! 配置IRQ为输入, 中断禁止, 低电平触发
